...
addEventHandler("onClientResourceStart", resourceRoot, function ()
	...
	for i, model in ipairs(skins) do
		if fileExists("tancerki/" .. tostring(model) .. ".dff") then
			local txd = engineLoadTXD("tancerki/"..tostring(model)..".txd")
			engineImportTXD(txd, model)
			local dff = engineLoadDFF("tancerki/"..tostring(model)..".dff")
	...
	if button == "g" then
		for i, ped in ipairs(peds) do
			if localPlayer:isWithinColShape(ped.colshape) then
				triggerServerEvent("crpg_club.throwMoney", resourceRoot, i)
				return
			end
		end
	end
