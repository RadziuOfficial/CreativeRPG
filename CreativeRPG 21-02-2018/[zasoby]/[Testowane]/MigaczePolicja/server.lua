	...
	if (id == 596) or (id == 597) then
		if(level == 1) then
			if(p_lights[veh] == 0) or(p_lights[veh] == nil) then
				p_pvar[veh] = 1
				p_lights[veh] = 1
				outputChatBox("#FFFFFFUruchomiles policyjne swiatla", thePlayer, 0, 200, 100, true)
				...
				function()
					...
					p_lights[veh] = 0
					outputChatBox("#FFFFFFZdezaktywowales swoje policyjne swiatla", thePlayer, 0, 200, 100, true)
			...
			elseif(level == 2) then
				if(p_lights[veh] == 0) or(p_lights[veh] == nil) then
				p_lights[veh] = 1
				outputChatBox("#FFFFFFUruchomiles swiatla poscigowe", thePlayer, 0, 200, 100, true)
				setVehicleOverrideLights ( veh, 2 )
				p_timer[veh] = setTimer(
				function()
					...
					p_lights[veh] = 0
					outputChatBox("#FFFFFFZdezaktywowales swiatla poscigowe", thePlayer, 0, 200, 100, true)
					killTimer(p_timer[veh])
					setVehicleLightState ( veh, 0, 0)
					setVehicleLightState ( veh, 1, 0)
					setVehicleLightState ( veh, 2, 0)
