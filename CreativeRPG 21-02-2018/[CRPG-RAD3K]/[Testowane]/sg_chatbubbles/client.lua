					...
					if tick-v.tick < timeVisible then
						...
						if getDistanceBetweenPoints3D(x, y, z, px, py, pz) < distanceVisible and isLineOfSightClear ( x, y, z, px, py, pz, true, not isPedInVehicle(v.player), false, true) then
							...
							if sx and sy then
								...
								if bubble then
									dxDrawImage ( sx-width/2-10, yPos - 63, width+16, 20, v.texture, nil, nil, tocolor(0, 94, 254, v.alpha) )
								else
									dxDrawRectangle(sx-width/2-10, yPos - 60, width+16, 20, tocolor(0, 0, 0, v.alpha))
								end
								dxDrawText(v.text, sx-width/2-2, yPos - 60, width, 20, tocolor( 255,255,255, v.alpha+50), 1, "default-bold", "left", "top", false, false, false, true)
							end
						end
					else
